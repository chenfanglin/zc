...
public class ConcurrentLRUHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
	 ...
	 * itself is a concurrently readable hash table.
	 */
	...
	*/
	...
	static final class Segment<K, V> extends ReentrantLock implements Serializable {
		 ...
		 * (final). All list additions are performed at the front of each bin. This
		 * makes it easy to check changes, and also fast to traverse. When nodes would
		 * otherwise be changed, new nodes are created to replace them. This works well
		 * for hash tables since the bin lists tend to be short. (The average length is
		 ...
		 * of volatiles to ensure that completed write operations performed by other
		 * threads are noticed. For most purposes, the "count" field, tracking the
		 * number of elements, serves as that volatile variable ensuring visibility.
		 ...
		 * example, no operation can reveal that the table has grown but the threshold
		 * has not yet been updated, so there are no atomicity requirements for this
		 ...
		 * containsValue, then we might have an inconsistent view of state so (usually)
		 * must retry.
		 */
		 ...
		 * to be null. This is possible only if a compiler happens to reorder a
		 * HashEntry initialization with its table assignment, which is legal under
		...
		void rehash() {
			 ...
			 * change. Statistically, at the default threshold, only about one-sixth of them
			 * need cloning when a table doubles. The nodes they replace will be garbage
			 * collectable as soon as they are no longer referenced by any reader thread
			 * that may be in the midst of traversing table right now.
			 */
				...
				if (e != null) {
					...
					if (next == null)
						...
						for (HashEntry<K, V> p = e; p != lastRun; p = p.next) {
							...
							HashEntry<K, V> newEntry = new HashEntry<K, V>(p.key, p.hash, n, p.value);
							// update by Noah
	 ...
	 *            并发级别
	 */
	public ConcurrentLRUHashMap(int segementCapacity, float loadFactor, int concurrencyLevel) {
	...
	public ConcurrentLRUHashMap() {
		this(DEFAULT_SEGEMENT_MAX_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL);
	}
	...
	public boolean isEmpty() {
		...
		/*
		 * We keep track of per-segment modCounts to avoid ABA problems in which an
		 * element in one segment was added and in another removed during traversal, in
		 * which case the table was never actually empty at any point. Note the similar
		 * use of modCounts in the size() and containsValue() methods, which are the
		 * only other methods also susceptible to ABA problems.
		 */
