	...
	hdlPeriod : function(evt){
		...
		if( monthPicker.Params.current ==  monthPicker.Constant.START_MONTH){
			...
			*/
			...
			that._conf.start_month = start.year+'-'+start.month;
			that._conf.end_month = end.year+'-'+end.month;
			//提交，调用回调函数
			that._conf.callback({startDate:start.year+'-'+start.month, endDate:end.year+'-'+end.month});
